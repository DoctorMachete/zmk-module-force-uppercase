 ...
 *
 * Steps:
 *   1. mask  = ZMK_SHIFT_MODS       → wipes all physical shift from report
 *   2. if report_shift needed        → inject via implicit (survives mask)
 *   3. zmk_hid_press/release         → update key state, no event bus
 *   4. zmk_endpoints_send_report     → send while modifiers are correct
 *   5. restore implicit + mask       → HID state identical to before
 *
 * Bypasses raise_zmk_keycode_state_changed_from_encoded entirely because
 * hid_listener would call zmk_hid_implicit_modifiers_press(ev->implicit=0)
 * overwriting our implicit before the report is sent.
 *
